...
Nedenstående figur viser det flowdiagram (1. version), som jeg udarbejdede for at kunne lave algoritme koden.
<img width="1335" height="824" alt="image" src="https://github.com/user-attachments/assets/facdb0c5-ae45-4879-9977-d30c259ffc33" />

# Datastrukturer, overvejelser og valg
  Eftersom applikationen vælger random elev (på indeksnummer) fra de forskellige lister, så er det ArrayList, som jeg anvender til dette.
  I fordelingsalgoritmen starter jeg dog med at hente alle valgfag, så der ikke skal laves opslag ned i databasen for hver gang en elev håndteres, når der skal tjekkes for plads på holdet. Disse valgfag bliver gemt i HashMap, hvor key er courseId, så der hurtigt kan laves opslag. Eftersom der godt nok kun er 8 forskellige valgfag, har HashMap contra ArrayList nok ikke den store performance betydning her. Men at alle valgfag er hentet inden, og der ikke skal laves database kald minimum tre gange pr elev, det har stor betydning for performance.
  Som I kan se, så har jeg én stor samlet service-klasse (AdministrationService), hvori bl.a. fordelingsalgoritmen og diverse hjælpemetoder findes. Almindeligvis vil jeg opdele i flere service-klasser alt efter hvad metoderne skal behandle, så det ikke bliver så uoverskueligt. Dette startede jeg også med, men fik så problemer i forhold til at benytte @Transactional og mulighederne i Jpa med at gemme direkte i DB, når der var ændring i en Entity. For at undgå at skulle tilføre manuel håndtering af save() i metoderne (og de deraf flere DB kald), så valgte jeg at indarbejde det hele i én service-klasse. Jeg har prøvet at lave opdeling med overskrifter og forskellige kommentarer for overblikket skyld.
