...
Processen (distributionGreedyWithFairness()) kan i korte træk beskrives som vist nedenstående, og mine forudsætninger til selve algoritmen er et udgangspunkt i minimum 8 elever og max 35 elever på et hold. Tanken er, at ved den efterfølgende manuelle tjek og restfordeling, så vil dette kunne overrules (ikke implementeret på nuværende tidspunkt).
## Algoritmens proces:
...
Eftersom applikationen vælger random elev fra de forskellige lister, så er det **ArrayList**, som jeg anvender til de forskellige elev lister.
